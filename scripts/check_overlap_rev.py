...
def extend_gtf_frame(pickle):
    ...
    for transcript in gtf_coords_file:
        ...
        for x_transcript in gtf_coords_file:
            if transcript.strand == "+":
                if x_transcript.overlaps(new_region):
                    transcript.add_segments(new_region, new_region_2)
                    yield transcript

            elif transcript.strand == "-":
                if x_transcript.overlaps(new_region_2):
                    transcript.add_segments(new_region, new_region_2)
                    yield transcript
...
def fetch_vectors(filename):
    ...
    for transcript in extend_gtf_frame(global_args.annotation_file):
        if any([transcript.attr.get('Name') in allowed_ids, transcript.get_name() in allowed_ids]):
            try:
                ...
                # allowed_ids.discard(transcript.attr.get("Name"))
                # allowed_ids.discard(transcript.get_name())
            except ValueError:
                pass
    ...
    if global_args.normalize == True:
        ...
        vector_array_term = vector_array_term[~np.all(
            vector_array_term == 0, axis=1)]
        vector_array_term = vector_array_term / \
            vector_array_term.sum(1)[:, np.newaxis]
...
def scale_labels():
    xlabels = []
    for x in range(-global_args.offset+1, global_args.offset+1, 10):
        if x % 10 == 0:
            xlabels.append(x)
        else:
            xlabels.append(" ")
...
def plot_results_start(bampath, bamname):
    ...
    plt.xticks(np.linspace(-global_args.offset, global_args.offset,
                           num=global_args.offset), labels, size="xx-small")

    plt.legend()
    plt.savefig(global_args.output_dir +
                "coverage_start_no_overlap/%s%s_no_overlap.pdf" % (bamname, norm))
    plt.close()
    ...
    plt.xticks(np.linspace(-global_args.offset, global_args.offset, num=global_args.offset),
               labels, size="xx-small")
    plt.legend()
    plt.savefig(global_args.output_dir +
                "coverage_term_no_overlap/%s%s_no_overlap.pdf" % (bamname, norm))
    plt.close()
...
def executable():
    ...
    for bampath, bamname in runall_samples(global_args.sample_dir):
        try:
            os.mkdir(global_args.output_dir + "coverage_start_no_overlap/")
            os.mkdir(global_args.output_dir + "coverage_term_no_overlap/")
        except:
            pass
