...
def plot_results_start(bampath, bamname):
    ...
    plt.xticks(np.linspace(-global_args.offset, global_args.offset, num=int(global_args.offset*2/10)),
               labels, size="xx-small")
    plt.axvline(len(metagene) - int(global_args.offset/10))
    plt.legend()
    plt.savefig(global_args.output_dir + "coverage_start_no_overlap/%s%s_no_overlap.pdf" % (bamname, norm))
    plt.close()
    ...
    plt.axvline(len(metagene) - int(global_args.offset/10))
    plt.legend()
    plt.savefig(global_args.output_dir + "coverage_term_no_overlap/%s%s_no_overlap.pdf" % (bamname, norm))
    plt.close()
