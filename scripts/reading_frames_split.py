...
def plot_results_start(bampath, bamname):
    ...
    plt.axvline(int(global_args.offset)/3, linewidth=1, linestyle="--", color="black")
    ...
    plt.plot(frames[0][1][2:], linewidth=1, color="green", label="Frame +1 (main)")
    plt.plot(frames[0][2][2:], linewidth=1, color="blue", label="Frame -1")
    plt.savefig(global_args.output_dir +
                "frames_coverage_start/ribosome_protection_%s%s.png" % (bamname, norm))
    plt.close()

    plt.grid(True, alpha=0.3)
    plt.axvline(len(frames[1][0][:-2]) - int(global_args.offset)/3, linewidth=1, linestyle="--", color="black")
    ...
    plt.plot(frames[1][1][:-2], linewidth=1, color="green", label="Frame +1 (main)")
    plt.plot(frames[1][2][:-2], linewidth=1, color="blue", label="Frame -1")
    plt.savefig(global_args.output_dir +
                "frames_coverage_term/ribosome_protection_%s%s.png" % (bamname, norm))
    plt.close()
